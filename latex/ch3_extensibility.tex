...
\begin{figure*}[t!]
    ...
    \begin{subfigure}[t]{0.5\linewidth}
        \centering
        \includegraphics[width=0.9\linewidth]{mission2/timecostcontours.png}
    \end{subfigure}%
    ~
    \begin{subfigure}[t]{0.5\linewidth}
        \centering
        \includegraphics[width=0.9\linewidth]{mission2/Csenscontours.png}
    \end{subfigure}
    \caption{We can gain intuition about the relative importance of different components
...
improvement before we can perform multimission design in Section~\ref{s:mm}.
We can see this by observing the sensitivities in Table~\ref{t:sens}.

\begin{footnotesize}
\captionof{table}{A selection of sensitivities of the model.}
\label{t:sens}
\begin{longtable}{llcl}
\toprule
Variable & Sensitivity & Units & Description \\ \midrule
\multicolumn{3}{l}{\textbf{Mission}} \\ \\
$Range$ & +1.1  & $~\mathrm{km}$ & aircraft range \\
$V_{min}$ & -0.67  & $~\mathrm{\tfrac{m}{s}}$ & takeoff speed \\
$C$ & +0.46  & $~\mathrm{\tfrac{1}{hr}}$ & hourly cost index \\
$W_p$ & +0.45  & $~\mathrm{N}$ & payload weight \\
$h_{cruise}$ & +0.12  & $~\mathrm{m}$ & minimum cruise altitude \\
 &  &  &  \\
\multicolumn{3}{l}{\textbf{Mission/SimPleAC}} \\ \\
$g$ & +0.54  & $~\mathrm{\tfrac{m}{s^{2}}}$ & gravitational acceleration \\
$\rho_f$ & -0.042  & $~\mathrm{\tfrac{kg}{m^{3}}}$ & density of fuel \\
 &  &  &  \\
\multicolumn{3}{l}{\textbf{Mission/SimPleAC/Engine}} \\ \\
$\eta_{prop}$ & -0.65  & $$ & propeller efficiency \\
$P_{shaft,ref}$ & -0.067  & $~\mathrm{kW}$ & reference MSL maximum shaft power \\
$W_{e,ref}$ & +0.044  & $~\mathrm{lbf}$ & reference engine weight \\
 &  &  &  \\
...
$W_{w_{coeff2}}$ & +0.12  & $~\mathrm{Pa}$ & wing weight coefficient 2 \\
$\tau$ & -0.11  & $$ & airfoil thickness to chord ratio \\
$N_{ult}$ & +0.07  & $$ & ultimate load factor \\
$W_{w_{coeff1}}$ & +0.07  & $~\mathrm{\tfrac{1}{m}}$ & wing weight coefficient 1 \\
 &  &  &  \\
\multicolumn{3}{l}{\textbf{Mission/SimPleACP/EngineP}} \\ \\
$BSFC$ & [ +0.16     +0.14     +0.14     +0.14    ]  & $~\mathrm{\tfrac{g}{\left(hr\cdot kW\right)}}$ & brake specific fuel consumption \\
\bottomrule
\end{longtable}
\end{footnotesize}
...
%quality of the optima. The mathematical guarantees and speed of \gls{GP} allow engineers to make observations
%about the Pareto frontier of a problem quickly, and improve the quality of the models.
...
Then in Section\ref{s:multimission},
we can see how improved modeling affects the results of a multimission model.
...
\begin{center}
    \begin{figure}
        ...
        \caption{$\frac{\BSFC}{\BSFC_{min}}$ versus $\frac{P_{shaft}}{P_{shaft,alt}}$ data fit. The green fit is
...
namely  $W_{f_{m_{2}}}$ and $t_{m_{1}}$, will diverge to numerical infinity.
...
\begin{enumerate}
    \item A single mission (named M1) minimizing fuel burn ($W_{f_m}$).
    \item A single mission (named M2) minimizing time cost ($\mathrm{C} \times t_{m}$)
    \item A multimission design performing M1 and M2, minimizing fuel burn over M1 ($W_{f_{m_1}}$), and mission
...
$V_{min}$ & \multicolumn{2}{c}{25} & $~\mathrm{\tfrac{m}{s}}$ & takeoff speed \\
    \caption{Inputs to the \textbf{\textit{Multimission}} model.}
    \label{t:mminputs}
\end{longtable}
\end{center}
\end{footnotesize}
...
\begin{center}
    \captionof{table}{Results of the single- and multi- mission solutions. Italicized figures
    have been post-processed.}
    \input{tables/mmtable.tex}
    \label{t:mmoutputs}
\end{center}

The multimission aircraft represents a compromise aircraft that gives adequate
...
Multimission design would be further strengthened by the application of
