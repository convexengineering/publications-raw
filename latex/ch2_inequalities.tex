...
Using the \gls{GP} form, since we can always upper-bound posynomials, and lower bound both posynomials
...
precision zero,
giving a non-sensical feasibility set shown in Figure~\ref{feas:posyleq}. In this section, I will
demonstrate methods to create \gls{GP}-compatible models that are adequately bounded to avoid such
singularities, which for the most part do not
exist in real physical models.

\section{Defining the design problem}

\gls{GP}s are amenable to solving a large variety of design problems (see~\cite{gp_intro} for an
...
Note that these are in now way presented in order of importance, which is reflective
...
parameters, and the others free variables. This decision is
can shape the whole model development process in all forms of optimization.

The decision of whether to keep variables free or fixed can be determined by many
...
of some of the variables as defined in Table~\ref{tab:vars_WandL}.
...
will likely be unbounded for a given model and run off the +$\infty$. This means that
performance-quantifying variables
that are not involved in the fundamental physics of a model will need to be in a monomial form,
or must be present in a posynomial objective
function as shown in Equation~\ref{e:posyobj} for boundedness.

\begin{equation}
    \mathrm{Objective} \geq \sum\limits^n_{i=1}\prod\limits^n_{i=1} c_{i,j} v^{k_{i,j}}
    \label{e:posyobj}
\end{equation}
...
The results are shown in Table~\ref{tab:WandL_unbounded}.
...
in the design space as possible. The intuition is that, we can almost always have more drag,
but the constraint will always be tight since all objectives will suffer.
...
The new variables are detailed in Table~\ref{tab:vars_TandD}.
...
One can post-process the results of the optimization problem to get complete geometry
as necessary. The new variables introduced to bound fuel volume are in Table~\ref{tab:vars_fuelmodel}.
...
In a segue, I will explain and motivate the use of signomial equalities,
...
the final set of variables in Table~\ref{tab:vars_wingstrcmodel}.
