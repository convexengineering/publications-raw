...
GPkit facilitates process by providing feedback to designers about
...
precision zero,
giving a non-sensical feasibility set shown in Figure~\ref{feas:posyleq}. In this section, we will
create models that are adequately bounded to avoid such singularities, which for the most part do not
...
that and can shape the whole model development process in all forms of optimization.
...
but it is likely that most objectives will suffer.
...
volume $V_{f_{fuse}}$ and the wing weight $W_w$ still being lower-unbounded. Next,
...
The new variables introduced to bound fuel volume are in Table~\ref{tab:vars_fuelmodel}.
