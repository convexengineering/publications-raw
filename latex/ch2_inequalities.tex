...
\footnotesize
\centering
\begin{supertabular}{ l c l c }
    \toprule
	Variable & Units & Description & Free \\
    \midrule
    $C_L$ & $$ & lift coefficient of wing & N \\
    $S$ & $~\mathrm{m^{2}}$ & total wing area & Y \\
    $V$ & $~\mathrm{\tfrac{m}{s}}$ & cruising speed & Y \\
    $V_{min}$ & $~\mathrm{\tfrac{m}{s}}$ & takeoff speed & N \\
    $W$ & $~\mathrm{N}$ & total aircraft weight & Y \\
    $W_f$ & $~\mathrm{N}$ & fuel weight & Y \\
    $W_w$ & $~\mathrm{N}$ & wing weight & Y \\
    $W_p$ & $~\mathrm{N}$ & payload weight & N \\
    \bottomrule
\end{supertabular}
