...
about the structure of a problem and the way that each variable is pressured
by a given objective function. I will derive a simple aircraft problem that will serve as
...
of the non-hierarchical nature of \gls{GP}s. In the basic example, I will choose not to model engines,
...
sure each variable is adequately constrained later. Each subsection is intended
...
the objective function. Without the pressure from the objective, any variables that
are not both upper- and lower- bounded will tend to run away. This is indicative usually
...
\subsection{Signomial constraints: adding fuel volume constraints}
...
In a segue, I will attempt to explain and motivate the use of signomial equalities,
...
and the sensitivities of the parameters are presented.
