...
\begin{tikzpicture}[node distance=1.5cm, align=center, scale=0.5]
    \node (start)        [activityStarts]               {Initialize variables};
    \node (guess)        [process, below of=start]      {Make an initial guess};
    \node (convexify)    [process, below of=guess]      {Convexify};
    \node (optimize)     [process, below of=convexify]  {Optimize convex problem};
...
%        \end{tikzpicture}
%        \caption{Gradient-based program}
%    \end{subfigure}
%    \begin{subfigure}[b]
%        \begin{tikzpicture}[scale=0.8]
%            \draw (0,0) ellipse (1.0 and 0.5)
%        \end{tikzpicture}
%        \caption{Convex program}
%    \end{subfigure}
